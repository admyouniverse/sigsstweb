...
<section class="content">
    ...
    <div class="row" style="margin-bottom: 15px; margin-top: 10px">
        <div class="col-md-6">
            ...
            <download-excel v-if="montou" class="btn btn-sm btn-secondary" :data="this.cores" name="cores.xls">
    ...
    <div v-if="montou" class="box box-info">
        ...
        <div class="box-body table-responsive">
            <table class="table table-hover table-striped" id="tabelaListagem">
                ...
                <tbody>
                    <tr v-for="cor in cores">
                        <td style="vertical-align: middle; width: 100px"><b>{{ cor.idCor }}</b></td>
                        <td style="text-align:justify">{{ cor.descricao }}</td>
                        <td style="text-align:justify;">{{ cor.observacao }}</td>
                        <td width="100px" style="vertical-align: middle" class="text-center">
                            <button type="button" @click="remover(cor)" class="btn btn-sm btn-outline-danger"><i class="fa fa-trash"></i>
                            </button>
                            <button type="button" @click="preencher(cor)" class="btn btn-sm btn-outline-primary"><i class="fas fa-pencil-alt"></i>
                            </button>
                        </td>
                    </tr>
                </tbody>
        ...
        <div class="row">
            <div class="col-md-2">
                <div class="form-group">
                    <label>ID</label>
                    <input type="text" disabled v-model="cor.idCor" class="form-control">
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    <label>Descrição</label>
                    <input type="text" v-model="cor.descricao" class="form-control">
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label>Observação</label>
                    <input type="text" v-model="cor.observacao" class="form-control">
                </div>
            </div>
        </div>
    </b-modal>
    <b-modal size="sm" :no-close-on-backdrop="true" centered :hide-header="true" :hide-footer="true" :no-close-on-esc="true" ref="modalCerteza" id="modalCerteza">
        <div class="row">
            <div class="col-md-12 text-center">
                <h3>Tem certeza que deseja excluir esta cor?</h3>
            </div>
        </div>
...
export default {
    ...
    mounted() {
        var that = this;
        axios.get('http://200.98.201.236:8080/ServicoSIGSSO/rest/cores').then(function (response) {
            that.cores = response.data;
            that.montou = true;
            setTimeout(function () {
