            ...
            <table class="table table-hover">
                <tbody>
                    <tr>
                        ...
                        <th style="width:150px">Inscrição Terceiros</th>
                        <th>Observação</th>
                        <th>Subcódigo</th>
                        <th style="width:110px">Ações</th>
                    </tr>
                    <tr style="cursor:pointer" @click="getFuncoes(ambiente)" v-for="ambiente in ambientes">
                        <td>{{ ambiente.idPpraAmbienteTrabalho }}</td>
                        <td>{{ ambiente.nomeAmbienteTrabalho }}</td>
                        <td>{{ ambiente.estabelecimento }}</td>
                        <td>{{ ambiente.inscricaoTerceiro }}</td>
    ...
    <b-modal size="xl" centered :no-close-on-backdrop="true" :hide-header-close="true" :no-close-on-esc="true" title="Detalhes do Ambiente" @ok="salvar" @cancel="fechar()" ok-title="Salvar" cancel-title="Cancelar" ref="myModal" id="modal1">
        ...
        <div v-if="selecionado || editar" class="row">
            <div class="col-md-3">
                <div class="form-group">
                    <label for="">ID</label>
                    <input v-model="ambiente.idPpraAmbienteTrabalho" type="text" class="form-control" disabled>
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group">
...
this.$root.$emit('montou', [this.idTabela]);
        ...
        getFuncoes: function(ambiente) {
            ...
            window.location.href= "/gro/" + this.ppra.idPpra + "/ambiente/" + ambiente.idPpraAmbienteTrabalho;
            // axios.get('/gro/ambiente/' + ambiente.idPpraAmbienteTrabalho + '/funcoes').then(function(response) {
            //     console.log(response.data);
