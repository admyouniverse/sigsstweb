...
export default {
    ...
    methods: {
        ...
        salvar() {
			...
			axios.post('/prontuario-medico/afastamento', {
				...
				axios.get(process.env.MIX_APP_API + 'ServicoSIGSSO/rest/prontuario-afastamentos/listaPorIdProntuario/' + that.prontuario.idProntuario).then(function (response) {
    ...
    mounted() {
        var that = this;
        axios.get(process.env.MIX_APP_API + 'ServicoSIGSSO/rest/prontuario-afastamentos/listaPorIdProntuario/' + this.prontuario.idProntuario).then(function (response) {
			that.afastamentos = response.data;
        });
